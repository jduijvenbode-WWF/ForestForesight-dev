...
for (tile in tiles) {
  ...
  t = t + 1
  input_files = list.files(paste0("D:/ff-dev/results/preprocessed/input/", tile ),"forestcount30p",full.names = T)
  ...
  for (file in input_files) {
    ...
    file.copy(file,file.path("D:/ff-dev/backupfc30p/",basename(file)))
