...
#
proc_dates = daterange("2022-06-01","2024-01-01")
...
for (proc_date in proc_dates) {
  proc_date = as.Date(proc_date)
  new_models = c()
  for (x in seq(1,length(countrynames))) {
  ...
  #  if (!file.exists(paste0(country,"_",proc_date,".tif"))) {
      ...
      if (((grepl("1_year_training", method)) & proc_date != proc_dates[1]) || any(new_models == group)) {
        ...
        tryCatch({
          b <- train_predict_raster(shape = shape,
                                    ...
                                    label_threshold = 1,
                                    accuracy_csv = "D:/ff-dev/results/accuracy_analysis/accuracy_202407.csv")
