...
for (kernel in kernels) {
  for (cost in cost_values) {
    ...
    result <- train_svm_params(train_data, valid_data, kernel = kernel, cost = cost)
    model_name <- paste("svm_kernel", kernel, "cost", cost, sep = "_")
    models_list[[model_name]] <- result
    saveRDS(list(model = result$model, preproc = result$preproc), file = paste("/models/svm", model_name))
  }
}
