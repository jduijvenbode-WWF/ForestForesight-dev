...
calculate_scores=function(predfile,groundtruthfile,pols,adddate=T,fmaskfile=NULL,days){
...
files=c(files[grep("2023",files)],files[grep("2024",files)])
files=files[-c(grep("2023-01",files),grep("2023-12",files))]
for(x in 1:length(files)){
  file=files[x]
  cat(paste0(file," (",x," out of",length(files),")\n"))
  blfile=gsub("groundtruth","input",gsub("groundtruth6m","timesinceloss",file))
  ...
  calcpols=calculate_scores(predfile = blfile,groundtruthfile = groundtruthfile,pols = pols,fmaskfile = fmaskfile,days=days)
