    ...
    with rasterio.open(src_path) as src:
        ...
        with rasterio.open(output_path, 'w', **meta) as dst:
            ...
            for ji, window in src.block_windows(1):
                ...
                # Define the destination window dimensions
                dst_window = rasterio.windows.Window(
                    col_off=int(window.col_off * (width / src.width)),
                    row_off=int(window.row_off * (height / src.height)),
                    width=max(1, int(window.width * (width / src.width))),
                    height=max(1, int(window.height * (height / src.height)))
                )

                resampled_edges = np.zeros(
                    (dst_window.height, dst_window.width), dtype=np.float32
                )
                ...
                reproject(
                    ...
                    src_crs=src.crs,
                    dst_transform=transform * transform.translation(
                ...
                # Multiply and write to file per window
                resampled_edges *= 100
                dst.write(resampled_edges, 1, window=dst_window)
