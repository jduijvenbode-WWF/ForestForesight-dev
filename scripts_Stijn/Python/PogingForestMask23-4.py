...
def adjust_forest_layers(loss_file, forestmask_file, output_dir, tile_info):
    ...
    loss_mask = np.where(((loss >=1) & (loss <= 22)), 1, 0)
    ...
    # Copy the metadata here, before leaving the with block
    ...
    # Save the modified forest mask for inspection
    modified_forestmask_path = os.path.join(output_dir, f"{tile_info}_forestmask_modified.tif")
   
    
    
    forestmask_2021_path = os.path.join(output_dir, f"{tile_info}_forestmask_2023.tif")
    forest_meta = src_forestmask.meta.copy()
    forest_meta.update(dtype='uint16', compress='LZW')
    with rasterio.open(forestmask_2021_path, 'w', **forest_meta) as dst:
        dst.write(modified_forestmask, 1)

    print("Save completed for 2024 forest mask.")
...
output_dir = r"D:\temp\ForestmaskJonasscript\2023"  # Updated output directory
...
for loss_file in loss_files[start_index:end_index]:
    ...
    if os.path.exists(forestmask_file):
        adjust_forest_layers(loss_file, forestmask_file, output_dir, tile_info)
    else:
