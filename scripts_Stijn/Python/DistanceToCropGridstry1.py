...
def distance_to_nearest_nonzero_geotiff(input_geotiff, output_geotiff):
    ...
    if np.all(input_array == 0):
        ...
        max_distance = np.max(initial_transform)
        scale_factor = 254 / max_distance
        normalized_transform = 254 - (scale_factor * initial_transform)
        
        # Directly set original non-zero values to 255
        final_transform = normalized_transform
        initial_transform[mask] = 255
